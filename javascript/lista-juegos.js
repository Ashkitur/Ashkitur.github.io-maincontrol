...
juegosList.style.gap = "30px 200px";


  data.forEach(juego => { 
    const col = document.createElement("div");
    col.innerHTML = `
     <div class="card h-100 d-flex flex-column shadow-sm juego-item"
     style="background-color: black; color: white; border-radius: 33px; overflow: hidden; width: 100%; max-width: 650px; min-width: 320px; padding: 20px 24px 24px 24px; box-sizing: border-box;">
  ...
  <h5 class="card-title text-left juego-nombre"
      style="font-size: 1.7rem; min-height: 3.6em; line-height: 1.25em; overflow-wrap: break-word; word-break: break-word; margin-bottom: 22px;">
    ${juego.nombre}
  </h5>

  <p class="card-text text-left" style="margin-bottom: 14px;">
    <b>Precio:</b> ₡${juego.precio}
  </p>

  <p class="card-text text-left" style="white-space: normal; overflow-wrap: break-word; word-break: break-word; margin-bottom: 22px;">
    <b>Plataformas:</b> ${juego.plataformas.join(", ")}
  </p>

  <div style="margin-top: auto; display: flex; gap: 16px; flex-wrap: wrap;">
    <button class="btn flex-fill" onclick="comprarJuegos(${juego.id})"
            style="background-color:#df6011; color:#fff; border-radius:15px; border:none; padding: 12px 18px; font-size: 1.05rem; min-width: 110px;">
      <i class="bi bi-cart-plus"></i> Comprar
    </button>

    <button class="btn flex-fill" onclick="detalleJuego(${juego.id})" data-id="${juego.id}"
            style="background-color:WHITE; color:BLACK; border-radius:15px; border:none; padding: 12px 18px; font-size: 1.05rem; min-width: 110px;">
